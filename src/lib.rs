...
mod test {
    ...
    #[test]
    fn pdf_linearized() -> Result<(), Box<dyn std::error::Error>> {
        // Create a new PDF-document and add a single page
        let pdf = Document::new()?;
        pdf.page_add()?;

        // Newly created PDFs are not linearized
        let linearized = pdf.is_linearized()?;
        assert!(
            !linearized,
            "Expected a newly created PDF to be non-linearized"
        );

        pdf.optimize()?;
        pdf.optimize_resource()?;

        // Optimized PDFs are linearized
        let linearized = pdf.is_linearized()?;
        assert!(linearized, "Expected a optimized PDF to be linearized");

        Ok(())
    }

    #[test]
    fn pdf_meta_info() -> Result<(), Box<dyn std::error::Error>> {
        // Create a new PDF-document and add a blank page
        let pdf = Document::new()?;
        pdf.page_add()?;

        // Verify that a new document has no Author metadata
        let author = pdf.get_meta_info("Author")?;
        assert!(
            author.is_empty(),
            "Expected empty author metadata for a newly created PDF"
        );

        // Set Author metadata
        pdf.set_meta_info("Author", "Test Author")?;

        // Verify that it can be read back
        let author = pdf.get_meta_info("Author")?;
        assert_eq!(
            author, "Test Author",
            "Meta information should match the value set"
        );
    ...
    #[test]
    fn pdf_digital_signatures() -> Result<(), Box<dyn std::error::Error>> {
